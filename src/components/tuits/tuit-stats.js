...
const TuitStats = ({
  ...
  const likeTheTuit = async () => {
    await LikeService.createLike(currentUser._id, tuit._id);
  };

  const deleteTheTuit = async () => {
    await LikeService.deleteLike(currentUser._id, tuit._id);
  };
  ...
  const addRetuit = () => {
    ...
    const addRetuitHelper = async () => {
      const retuitCreated = await RetuitService.createRetuit(
        currentUser._id,
        tuit._id
      );
      const createdTuit = await TuitService.createTuitByUserId(
        currentUser._id,
        { tuit: tuit.tuit }
      );
      console.log(
        "retuitCreated: " +
          JSON.stringify(retuitCreated) +
          " createdTuit: " +
          JSON.stringify(createdTuit)
      );
  ...
  useEffect(() => {
    const findLikeCountAndIsTuitLiked = async () => {
      const likedData = await LikeService.findUsersThatLikeTheTuitByTuidId(
        tuit._id
      );
    ...
    const findRetuitCount = async () => {
      const retuitData = await RetuitService.findUsersThatRetuitTheTuitByTuidId(
        tuit._id
      );
    ...
    <>
      {JSON.stringify(currentUser) !== "{}" && (
        <div className="row mt-2">
          ...
          <div className="col">
            {Bookmark(tuit, currentUser, index, deleteBookmark)}
          </div>
        </div>
      )}
      {JSON.stringify(currentUser) == "{}" && (
